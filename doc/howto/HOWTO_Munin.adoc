...
TODO Check the following and adjust for multiple jobs:

Optionally backup the existing configuration file by moving the contents of 
link:https://github.com/xtuml/munin/tree/main/deploy/plus2json-deployed/generated-config[this directory] 
...
. `python plus2json.pyz Tutorial_1.puml --job -p | python -m json.tool > munin/deploy/plus2json-deployed/generated-config/tutorial_1.json` 
- this generates the job definition and loads it into the appropriate directory
. `python plus2json.pyz Tutorial_1.puml --audit_event_data -p |  python -m json.tool > munin/deploy/plus2json-deployed/generated-config/tutorial_1_aedata.json` 
- this generates the audit event data associated with the job definition and loads it into the appropriate directory
. `python plus2json.pyz Tutorial_1.puml --aeo_config -p |  python -m json.tool > munin/deploy/plus2json-deployed/generated-config/config.json` 
- this loads the master config file which sets a number of behavioural parameters and references each of the jobs
...
. `python plus2json.pyz Tutorial_1.puml --aesim_config -p | python -m json.tool > munin/deploy/aesim-config/plus2json/Tutorial_1.json` 
- this creates a job definition file for Tutorial_1 for consumption by the AE Simulator
. `python plus2json.pyz Tutorial_1.puml --aesim_test -p | python -m json.tool > munin/deploy/aesim-config/plus2json/plus2json-test-specification.json` 
