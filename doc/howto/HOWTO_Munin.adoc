...
`python plus2json.pyz Tutorial_1.puml` this checks the syntax of the puml file.
`python plus2json.pyz Tutorial_1.puml --job -p | python -m json.tool > munin/deploy/plus2json-deployed/generated-config/tutorial_1.json` this generates the job definition and loads them into the appropriate folder
`python plus2json.pyz Tutorial_1.puml --audit_event_data -p |  python -m json.tool > munin/deploy/plus2json-deployed/generated-config/tutorial_1_aedata.json` this generates the audit event data associated with the job definition and loads them into the appropriate folder
`python plus2json.pyz Tutorial_1.puml --aeo_config -p |  python -m json.tool > munin/deploy/plus2json-deployed/generated-config/config.json` this loads the master config file which references each of the job files

Additionally if the simulator is to be used also execute these:

`python plus2json.pyz Tutorial_1.puml --aesim_config -p | python -m json.tool > munin/deploy/aesim-config/plus2json/Tutorial_1.json` this creates a job definition file for Tutorial_1 for consumption by the AE Simulator
`python plus2json.pyz Tutorial_1.puml --aesim_test -p | python -m json.tool > munin/deploy/aesim-config/plus2json/plus2json-test-specification.json` this creates an example dispatch file for Tutorial_1 for consumption by the AE Simulator

=== Load the Protocol Verifier configuration files

TODO: - Check this is right:

==== Configuration of Protocol Verifier behaviour



To configure the Protocol Verifier: Backup the existing configuration file by moving
link:https://github.com/xtuml/munin/deploy/config/config.json[this] to a backup location of your choice.
Check that the output file from plus2json --aeo_config is named config.json. Move this file, replacing the existing file, to
link:https://github.com/xtuml/munin/deploy/config/[here]

There is a single config.json file which contains a number of configuation parameters and a list of the job specification
names together with individual job configuation parameters. Each job definition also has its own file containing the 
sequence and event definitions relevant to that job definition. Further, if the job definition requires any supplemantary 
audit event data then an additional configuration file defining the audit event data definitions for that job definition 
will be included.


.Click to see more details on the `deploy/config/config.json` config file format
[%collapsible]
====
...
TODO There are parameters present in the following file that are also
present in the `deploy/config/config.json` file described above

There are a set of configuation parameters that allow the reception
of files by the Protocol Verifier to be tuned.

.Click to see more details on the `deploy/reception-config/config.json` config file format
[%collapsible]
====

This configuration file contain some items that may be adjusted to manage the
the reception of events by the digital twin. The following is a list of the 
configuration items that can be adjusted and there description are as follows:
...
*SchemaValidate* - This is a boolean that determines whether the incoming
files should be validated against the defined schema.

*SchemaValidateFrequency* - This is an integer that indicates the frequency 
with which schema validation should be performed e.g. a value of 10 states that
every 10th file processed should be validated when the schemaValidate flag is set to true.
This is provided because performing schema validation is a time consuming process and 
this field allows tuning of how big an impact this has.
...
TODO Remove the following sentence and link in favour of separate links to individual diagrams at the appropriate 
...
TODO Check and fix the following
...
TODO The following won't work because feeding a job through that has already
been processed results in a `Duplicate Job Id identified` error
...
. link:images/04_drag_and_drop.gif[Drag and drop] `TODO.json` from the
...
TODO Change to the newer aer+aeo_svdc deployment scheme.
...
TODO Is the following still correct and/or useful?
...
TODO Update the following refer to terms in the actual data
- HighOutput and LowOutput have been removed
