...
PLUS domain specific language is based on PlantUML sequence diagrams. PlantUML
can be downloaded https://plantuml.com/starting[here] or installed through your
...
from the PlantUML sequence diagram syntax. A full description and guide for PLUS
can be found https://github.com/xtuml/munin/blob/main/doc/howto/PLUS_guide.adoc[here].

==== PLUS Processor

"PLUS Processor" refers to the application which processes PLUS files (textual
sequence diagrams) and produces JSON configuration files consumable by the
protocol verifier.
...
==== PLUS Processor domains

===== Statement Parser

The Statement Parser domain is responsible for parsing the PLUS files, verifying
that all syntax is correct and organizing them into a sequence of statements.
These statements are fed to the Sequence Processor one at a time to build up the
job definition.

===== Sequence Processor

The Sequence Processor domain reads statements from the Statement Parser one at
a time and interprets them into a structured job definition. It then serializes
this job definition to JSON.
