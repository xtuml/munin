...
. Step 5
  .. TODO Generalise audit event reception to remove the assumption that all
     jobIDs in a received message will be homogeneous.  Cycle through all
     audit events in the message creating needed audit event instances and
     appending audit events to lists of existing jobs.
  ...
  .. TODO `AuditEvent.auditEvent` and `AuditEvent.auditEvents` contain the
     entire list of events, which can be very long.  Shorten `auditEvent`
     somehow.  It is still needed for unit tests.

===== 5.1.1.1

  parse input
  if object
    jsonObject = JSON::get_objectrootJsonElement)
    jsonArray = jsonArray & jsonObject
  elsif array
    jsonObject = first object
    jsonArray = JSON::get_array(rootJsonElement)
  else
    // re-detect problem below
  end if

  if jsonObject'length > 0 and contains("jobId") then
    auditEventReception := jobManager -> R26
    for jsonElement in jsonArray loop
      jsonObject := get_object(jsonElement)
      if jsonObject'length > 0 and contains("jobId") then
        // create job (if needed) and audit event
        jobid = get_jobid()
        job = find job(jobid)
        if job = null
          create job
          create unassigned job
          link job to unassigned job
          add unassigned job to unassigned job list
        end if
        create acceptedAuditEvent
        link to job
    end loop

acceptedAuditEvents
for rawAuditEvent in auditEventArray
  jobId = extract job Id
  find acceptedAuditEvent in acceptedAuditEvents where (jobId)
  if null
    create acceptedAuditEvent


dictionary_of_JSONArray:  dictionary of JSON::JSONArray
for ae in rawAuditEvents loop
  jsonObject := JSON::get_object(ae)
  jobId := JSON::get_string(jsonObject["jobId"]);
  dictionary_of_JSONArray[jobId] := dictionary_of_JSONArray[jobId] & jsonObject
