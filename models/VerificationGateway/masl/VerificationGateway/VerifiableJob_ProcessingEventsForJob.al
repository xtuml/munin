...
begin
  ...
  if previousInstrumentationEvent /= null then
    ...
    // previousInstrumentationEvent by assigning its assignedPreviousEventId to the clientEventId of the 
    // previousInstrumentationEvent and then re-link R2 and link R3 in readiness for the next event.
    newInstrumentationEvent.assignedPreviousEventKey := previousInstrumentationEvent.eventKey;
