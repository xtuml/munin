domain IStore is
  ...
  public service persistInvariant ( invariantName: in string,
  ...
  private service testInvariantStore (); pragma test_only( true ); pragma scenario( 2 );   
  ...
  object InvStore is
    ...
    transition is
      Non_Existent ( loadStore => Cannot_Happen,
                     checkStore => Cannot_Happen );       
      Created ( loadStore => StoreLoaded,
                checkStore => Cannot_Happen );       
      StoreLoaded ( loadStore => StoreLoaded,
                    checkStore => StoreChecked );       
      StoreChecked ( loadStore => StoreLoaded,
                     checkStore => StoreChecked );       
    end transition;
