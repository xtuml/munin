...
DOCKER_RUN="docker compose -f ../../bin/docker-compose.yml run --service-ports -e AUDIT_EVENT_SCHEMA_PATH=${AUDIT_EVENT_SCHEMA_PATH} -e RECEPTION_CONFIG_SCHEMA_PATH=${RECEPTION_CONFIG_SCHEMA_PATH} -v /${PWD}:/work run"
