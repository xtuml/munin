...
if [[ $# -ge 3 ]] ; then
  ...
  echo "../tests/PumlForTesting/PumlRegression/SimpleSequenceJob.puml" | xargs $P2J --play --msgbroker localhost:9092 --topic $RECEPTION_TOPIC --shuffle --rate $EVENTS_PER_SECOND --num-events $PREPOPULATION_QUANTITY
  echo "press ENTER"
  read a
  echo "Done."
fi
